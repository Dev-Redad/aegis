...
import os, logging, time, random, re, unicodedata, html, threading, queue, json
from datetime import datetime, timedelta, timezone
from urllib.parse import quote
from urllib.request import Request as UrlRequest, urlopen
from urllib.error import URLError, HTTPError

from telegram import Update, ParseMode, InlineKeyboardButton, InlineKeyboardMarkup, Bot, InputMediaPhoto


class StyledInlineKeyboardButton(InlineKeyboardButton):
    """InlineKeyboardButton + Bot API 9.4's `style` and `icon_custom_emoji_id`
    fields. python-telegram-bot 13.x predates that API version and its
    InlineKeyboardButton uses fixed __slots__ that silently discard unknown
    constructor kwargs, so plain InlineKeyboardButton(style=...) does nothing.
    Subclassing with our own __slots__ makes PTB's generic slot-walking
    to_dict() pick these up as if they were native fields."""
...
# === Crypto payments (NOWPayments, USDT-BEP20 only) ===
NOWPAYMENTS_API_KEY = "P389TDM-2WEMZNG-JH3G0MG-HYJ53AX"
NOWPAYMENTS_INVOICE_URL = "https://api.nowpayments.io/v1/payment"
NOWPAYMENTS_STATUS_URL = "https://api.nowpayments.io/v1/payment/{payment_id}"
CRYPTO_PAY_CURRENCY = "usdtbsc"  # USDT on BEP20/BSC only
# Button styling (Bot API 9.4+). Custom emoji only renders if the bot is
# eligible to use custom emoji (Fragment-purchased username or owner Premium) --
# otherwise Telegram just ignores icon_custom_emoji_id and shows the button plain.
BUTTON_ICON_UPI = "5296467211735539812"
BUTTON_ICON_CRYPTO = "6028584717081645421"
CRYPTO_POLL_SECONDS = 15
DEFAULT_USD_INR_RATE = 96.0      # fallback used only if live-rate fetch fails and no custom rate is set
FX_RATE_CACHE_MINUTES = 30

# Payments & housekeeping
PAY_WINDOW_MINUTES = 10  # shared window for both UPI and crypto (payment detection can lag)
GRACE_SECONDS = 10
...
MONGO_URI = os.getenv(
    "MONGO_URI",
    "mongodb+srv://test:test@cluster0.ixul29b.mongodb.net/?appName=Cluster0"
)
...
c_sessions.create_index([("qr_message_id", ASCENDING)])
c_sessions.create_index([("crypto.payment_id", ASCENDING)])
...
def _is_not_modified_error(e: Exception) -> bool:
    return isinstance(e, BadRequest) and "not modified" in str(e).lower()

def _edit_ignore_unmodified(edit_fn, *args, **kwargs):
    """Telegram rejects edits whose content is byte-identical to the current
    message ('Message is not modified') -- e.g. an admin re-clicking the
    already-selected option. Treat that as success; log any other failure
    instead of crashing the dispatcher."""
    try:
        return edit_fn(*args, **kwargs)
    except Exception as e:
        if not _is_not_modified_error(e):
...
def record_earning(admin_id: int, amount: float, order_id: str, item_id: str, user_id: int, payment_method: str = "upi"):
...
def get_crypto_stats():
    """Proceeds processed via crypto (payment_method == 'crypto'), across all admins."""
    try:
        def _agg(match):
            result = list(c_earnings.aggregate([
                {"$match": match},
                {"$group": {"_id": None, "total": {"$sum": "$amount"}, "count": {"$sum": 1}}}
            ]))
            return (result[0]["total"], result[0]["count"]) if result else (0.0, 0)

        total, total_count = _agg({"payment_method": "crypto"})
        today, today_count = _agg({"payment_method": "crypto", "date": today_ist_str()})
        week, week_count = _agg({"payment_method": "crypto", "week_start": get_week_start_date().isoformat()})

        return {
            "total": total, "total_count": total_count,
            "today": today, "today_count": today_count,
            "week": week, "week_count": week_count,
        }
    except Exception as e:
        log.error(f"Failed to get crypto stats: {e}")
        return {"total": 0.0, "total_count": 0, "today": 0.0, "today_count": 0, "week": 0.0, "week_count": 0}
...
# ========== Crypto payments (NOWPayments, USDT-BEP20) ==========
# NOWPayments sits behind a WAF that blocks requests with Python's default
# urllib User-Agent ("Python-urllib/3.x") as bot traffic -- a plain browser-like
# UA plus an Accept header is required to get past it.
_NOWPAYMENTS_HEADERS = {
    "x-api-key": NOWPAYMENTS_API_KEY,
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
    "Accept": "application/json",
}

def _http_json(method: str, url: str, headers: dict, payload: dict = None, timeout: float = 15.0):
...
def crypto_payments_enabled() -> bool:
    return bool(cfg("crypto_enabled", False))

def _fetch_live_usd_inr_rate():
    """Best-effort live USD->INR rate from a couple of free, keyless FX APIs."""
    try:
        data = _http_json("GET", "https://api.frankfurter.app/latest?from=USD&to=INR", headers={"Accept": "application/json"}, timeout=8.0)
        rate = float((data.get("rates") or {}).get("INR"))
        if rate > 0: return rate
    except Exception as e:
        log.debug(f"frankfurter FX fetch failed: {e}")
    try:
        data = _http_json("GET", "https://open.er-api.com/v6/latest/USD", headers={"Accept": "application/json"}, timeout=8.0)
        rate = float((data.get("rates") or {}).get("INR"))
        if rate > 0: return rate
    except Exception as e:
        log.debug(f"er-api FX fetch failed: {e}")
    return None

def get_usd_inr_rate() -> float:
    """Returns the USD->INR rate to lock into a new checkout session, per the
    owner's configured mode (auto-live, cached, or a fixed custom rate)."""
    mode = cfg("crypto_rate_mode", "auto")
    if mode == "custom":
        try:
            v = float(cfg("crypto_custom_rate", DEFAULT_USD_INR_RATE))
            if v > 0: return v
        except Exception:
            pass
        return DEFAULT_USD_INR_RATE

    cache = cfg("fx_usd_inr_cache") or {}
    try:
        cached_at = datetime.fromisoformat(cache.get("ts"))
        if not cached_at.tzinfo: cached_at = cached_at.replace(tzinfo=UTC)
        fresh = (datetime.now(UTC) - cached_at) < timedelta(minutes=FX_RATE_CACHE_MINUTES)
    except Exception:
        fresh = False
    if fresh and cache.get("rate"):
        return float(cache["rate"])

    live = _fetch_live_usd_inr_rate()
    if live:
        set_cfg("fx_usd_inr_cache", {"rate": live, "ts": datetime.now(UTC).isoformat()})
        return live
    if cache.get("rate"):
        return float(cache["rate"])  # stale but better than nothing
    return DEFAULT_USD_INR_RATE

def inr_to_usdt(inr_amount: float, fx_rate: float) -> float:
    if fx_rate <= 0: fx_rate = DEFAULT_USD_INR_RATE
    return round(float(inr_amount) / fx_rate, 2)

def nowpayments_create_payment(usdt_amount: float, order_id: str, description: str):
    """Creates a NOWPayments mono-currency (USDT-BEP20 -> USDT-BEP20) payment.
    Returns (payment_id, pay_address, pay_amount) or (None, None, None) on failure."""
    payload = {
    ...
    headers = dict(_NOWPAYMENTS_HEADERS, **{"Content-Type": "application/json"})
    try:
        resp = _http_json("POST", NOWPAYMENTS_INVOICE_URL, headers=headers, payload=payload, timeout=15.0)
    except (HTTPError, URLError) as e:
        log.error(f"NOWPayments create payment request failed: {e}")
        return None, None, None
    except Exception as e:
        log.error(f"NOWPayments create payment unexpected error: {e}")
        return None, None, None
    return resp.get("payment_id"), resp.get("pay_address"), resp.get("pay_amount")

def nowpayments_get_status(payment_id: str):
    """Returns the NOWPayments status string for a payment_id (e.g. 'waiting',
    'confirming', 'finished', 'expired'), or None on failure."""
    url = NOWPAYMENTS_STATUS_URL.format(payment_id=payment_id)
    try:
        resp = _http_json("GET", url, headers=_NOWPAYMENTS_HEADERS, timeout=15.0)
    except Exception as e:
        log.warning(f"NOWPayments status check failed for {payment_id}: {e}")
        return None
    return resp.get("payment_status")
...
def start_purchase(ctx: CallbackContext, chat_id: int, uid: int, item_id: str):
    ...
    session = {
        ...
        "created_at": created, "admin_id": admin_id,
        "hard_expire_at": created + timedelta(minutes=PAY_WINDOW_MINUTES, seconds=GRACE_SECONDS),
        "status": "pending", "active_view": "upi",
        # Locked once at session creation -- never re-picked/re-fetched on toggle.
        "fx_rate": get_usd_inr_rate() if crypto_payments_enabled() else None,
        "crypto": None,
    }

    photo_url, caption, kb = _upi_view_content(session)
    sent = ctx.bot.send_photo(chat_id=chat_id, photo=photo_url, caption=caption, parse_mode=ParseMode.MARKDOWN, reply_markup=kb)
    session["qr_message_id"] = sent.message_id
    c_sessions.insert_one(session)
...
def _upi_view_content(session: dict):
    """Returns (photo_url, caption, keyboard) for the UPI QR view of a checkout session."""
    amt = session["amount"]
    chosen_upi = session["upi_id"]
    display_amt = fmt_inr(amt)
    uri = build_upi_uri(amt, f"order_uid_{session['user_id']}", chosen_upi)
    caption = (
    ...
    if crypto_payments_enabled():
        kb = InlineKeyboardMarkup([[StyledInlineKeyboardButton(
            "Pay with USDT (BEP20)", callback_data="paytoggle:crypto",
            style="success", icon_custom_emoji_id=BUTTON_ICON_CRYPTO,
        )]])
    return qr_url(uri), caption, kb

def _crypto_view_content(session: dict):
    """Returns (photo_url, caption, keyboard) for the crypto QR view. Assumes
    session['crypto'] is already populated (invoice created)."""
    c = session.get("crypto") or {}
    pay_amount = c.get("pay_amount")
    pay_address = c.get("pay_address")
    caption = (
        "*Pay with USDT (BEP20 / BSC network)*\n\n"
        f"Amount: `{pay_amount}` USDT\n"
        f"Address: `{pay_address}`\n\n"
        "Instructions:\n"
        "• Send USDT on the BEP20 (BSC) network ONLY — other networks will be lost\n"
        f"• Send the exact amount above within {PAY_WINDOW_MINUTES} minutes\n"
        "Verification is automatic. Delivery right after payment."
    )
    kb = InlineKeyboardMarkup([[StyledInlineKeyboardButton(
        "Pay with INR (UPI)", callback_data="paytoggle:upi",
        style="success", icon_custom_emoji_id=BUTTON_ICON_UPI,
    )]])
    return qr_url(pay_address), caption, kb
...
def on_channel_post(update: Update, context: CallbackContext):
    ...
    for s in matches:
        # Atomic claim: if a concurrent crypto confirmation already grabbed this
        # session (buyer paid the USDT address instead), back off instead of
        # double-delivering.
        ...
        try:
            _fulfill_paid_session(context, s, s.get("amount", amt), "upi")
        except Exception as e:
            _alert_fulfillment_failure(context, s, "upi", e)
...
def _alert_fulfillment_failure(context: CallbackContext, session: dict, payment_method: str, error: Exception):
    """A payment was confirmed but something inside delivery/earnings/receipt
    blew up mid-way (e.g. a transient DB or Telegram hiccup) -- log loudly and
    tell the owner, since the caller still cleans up the session/lock right
    after this (so the amount doesn't stay stuck forever) and there's no
    automatic retry. Manual delivery via the Order ID / user ID here is the
    fallback."""
    log.error(
        f"Fulfillment failed ({payment_method}) for user={session.get('user_id')} "
        f"item={session.get('item_id')} amount={session.get('amount')}: {error}",
        exc_info=error,
    )
    try:
        context.bot.send_message(
            OWNER_ID,
            "⚠️ *Payment confirmed but delivery/earning recording crashed.*\n"
            f"Method: `{payment_method}`\n"
            f"Buyer: `{session.get('user_id')}`\n"
            f"Item: `{session.get('item_id')}`\n"
            f"Amount: ₹{session.get('amount')}\n"
            f"Error: `{error}`\n"
            "Check logs and deliver manually if the buyer didn't receive their item.",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception:
        pass

def _fulfill_paid_session(context: CallbackContext, session: dict, amount_paid: float, payment_method: str):
    """Delivers the product, records the order/earning and receipt, and
    schedules cleanup for a session that has just been confirmed paid (via
    either UPI amount-matching or a confirmed crypto payment). Caller is
    responsible for the atomic claim guard and session/lock cleanup."""
    uid = session["user_id"]; chat_id = session["chat_id"]; item_id = session["item_id"]

    try:
        confirm_msg = context.bot.send_message(chat_id, "✅ Payment received. Delivering your item…")
    ...
    deliver_ids = deliver(context, uid, item_id, return_ids=True, notify_on_fail=False)
    ...
    admin_id = session.get("admin_id", OWNER_ID)
    if admin_id and amount_paid > 0:
        record_earning(admin_id, amount_paid, order_id, item_id, uid, payment_method=payment_method)
    ...
    if "channel_id" in prod:
        try:
            c_orders.update_one(
                {"user_id": uid, "channel_id": int(prod["channel_id"])},
                {"$set": {
                    "item_id": item_id, "paid_at": datetime.now(UTC), "status": "paid",
                    "order_id": order_id, "amount": float(amount_paid),
                    "admin_id": admin_id, "payment_method": payment_method,
        ...
        try:
            ...
            r = context.bot.send_message(uid, receipt_text, parse_mode=ParseMode.MARKDOWN)
            c_orders.update_one(
                {"user_id": uid, "channel_id": int(prod["channel_id"])},
        ...
        if not deliver_ids:
            ...
            txt = "⚠️ This Channel is not available right now. Please contact support"
            txt += f" {sup}." if sup else "."
            try: context.bot.send_message(uid, txt)
    ...
    if ids_to_delete:
        context.job_queue.run_once(
            _auto_delete_messages,
            timedelta(minutes=DELETE_AFTER_MINUTES),
            context={"chat_id": chat_id, "message_ids": ids_to_delete},
            name=f"del_{uid}_{int(time.time())}"
        )

# ---- Crypto payment toggle (same message, edited in place) ----
def pay_toggle_cb(update: Update, context: CallbackContext):
    q = update.callback_query
    data = q.data or ""
    m = re.match(r"^paytoggle:(upi|crypto)$", data)
    if not m:
        q.answer()
        return
    target = m.group(1)
    q.answer()  # answer immediately so the button doesn't spin while we hit the network

    msg = q.message
    # message_id is only unique per chat -- must scope by chat_id or two buyers
    # with the same small sequential message id would collide.
    session = c_sessions.find_one({"chat_id": msg.chat_id, "qr_message_id": msg.message_id, "status": "pending"})
    if not session:
        try:
            msg.edit_caption("⌛ This session has expired. Send /start to try again.")
        except Exception:
            pass
        return

    if target == "crypto" and not crypto_payments_enabled():
        return  # button shouldn't be visible in this case; ignore a stale click

    if target == "crypto" and not session.get("crypto"):
        fx_rate = session.get("fx_rate") or get_usd_inr_rate()
        usdt_amount = inr_to_usdt(session["amount"], fx_rate)
        order_ref = f"{session['user_id']}_{session['item_id']}_{int(time.time())}"
        payment_id, pay_address, pay_amount = nowpayments_create_payment(
            usdt_amount, order_ref, f"{session['item_id']} for user {session['user_id']}"
        )
        if not payment_id or not pay_address:
            try:
                context.bot.send_message(msg.chat_id, "⚠️ Crypto payment is temporarily unavailable. Please use UPI.")
            except Exception:
                pass
            return

        crypto_block = {
            "payment_id": payment_id, "pay_address": pay_address, "pay_amount": pay_amount,
            "created_at": datetime.now(UTC),
        }
        c_sessions.update_one({"_id": session["_id"]}, {"$set": {"crypto": crypto_block, "active_view": "crypto"}})
        session["crypto"] = crypto_block
        session["active_view"] = "crypto"

        context.job_queue.run_repeating(
            _poll_crypto_payment,
            interval=CRYPTO_POLL_SECONDS,
            first=CRYPTO_POLL_SECONDS,
            context={"session_key": session["key"]},
            name=f"crypto_poll_{session['key']}",
        )
    else:
        c_sessions.update_one({"_id": session["_id"]}, {"$set": {"active_view": target}})
        session["active_view"] = target

    photo_url, caption, kb = (_crypto_view_content(session) if target == "crypto" else _upi_view_content(session))
    try:
        context.bot.edit_message_media(
            chat_id=msg.chat_id,
            message_id=msg.message_id,
            media=InputMediaPhoto(media=photo_url, caption=caption, parse_mode=ParseMode.MARKDOWN),
            reply_markup=kb,
        )
    except Exception as e:
        if not _is_not_modified_error(e):  # rapid double-tap of the same button
            log.error(f"Toggle view edit failed: {e}")

def _poll_crypto_payment(context: CallbackContext):
    session_key = context.job.context["session_key"]
    session = c_sessions.find_one({"key": session_key, "status": "pending"})
    if not session or not session.get("crypto"):
        context.job.schedule_removal()
        return

    hard_expire_at = session.get("hard_expire_at")
    if hard_expire_at and not hard_expire_at.tzinfo:
        hard_expire_at = hard_expire_at.replace(tzinfo=UTC)
    now = datetime.now(UTC)

    payment_id = session["crypto"]["payment_id"]
    status = nowpayments_get_status(payment_id)

    if status == "finished":
        # Atomic claim: back off if the UPI rail already fulfilled this session.
        claimed = c_sessions.find_one_and_update(
            {"key": session_key, "status": "pending"},
            {"$set": {"status": "fulfilling"}}
        )
        context.job.schedule_removal()
        if not claimed:
        ...
        qr_mid = session.get("qr_message_id")
        if qr_mid:
            try: context.bot.delete_message(chat_id=session["chat_id"], message_id=qr_mid)
            except Exception as e: log.debug(f"Delete crypto QR failed: {e}")

        try:
            _fulfill_paid_session(context, session, session.get("amount", 0.0), "crypto")
        except Exception as e:
            _alert_fulfillment_failure(context, session, "crypto", e)
        c_sessions.delete_one({"_id": session["_id"]})
        release_amount_key(session["amount_key"])
        return

    if status in ("failed", "expired", "refunded") or (hard_expire_at and now >= hard_expire_at):
        context.job.schedule_removal()
        # Don't stomp a session the UPI rail just claimed in the same instant.
        if not c_sessions.find_one({"key": session_key, "status": "pending"}):
            return
        try:
            context.bot.edit_message_caption(
                chat_id=session["chat_id"], message_id=session["qr_message_id"],
                caption="⌛ Payment window expired. Send /start to try again.",
            )
        except Exception:
            pass

def _resume_pending_crypto_polls(context: CallbackContext):
    """Re-attach polling jobs for crypto sessions left 'pending' across a bot restart."""
    now = datetime.now(UTC)
    for session in c_sessions.find({"status": "pending", "crypto": {"$ne": None}}):
        hard_expire_at = session.get("hard_expire_at")
        if hard_expire_at and not hard_expire_at.tzinfo:
            hard_expire_at = hard_expire_at.replace(tzinfo=UTC)
        if hard_expire_at and now >= hard_expire_at:
            continue  # left for Mongo's TTL index to sweep
        context.job_queue.run_repeating(
            _poll_crypto_payment,
            interval=CRYPTO_POLL_SECONDS,
            first=1,
...
def _render_settings_text():
    ...
    lines.append(f"*Crypto payments (USDT-BEP20):* {'🟢 Enabled' if crypto_payments_enabled() else '🔴 Disabled'}")
    lines.append("")
...
def _settings_keyboard():
    ...
    rows = [
        ...
        [InlineKeyboardButton("🪙 Crypto Management", callback_data="cfg:cryptomgmt")],
    ]
...
def _crypto_mgmt_text() -> str:
    enabled = crypto_payments_enabled()
    mode = cfg("crypto_rate_mode", "auto")
    stats = get_crypto_stats()
    lines = ["*Crypto Management (USDT-BEP20)*", ""]
    lines.append(f"*Status:* {'🟢 Enabled' if enabled else '🔴 Disabled'}")
    if mode == "custom":
        rate = float(cfg("crypto_custom_rate", DEFAULT_USD_INR_RATE))
        lines.append(f"*Rate:* custom — ₹{rate:.2f} / $1")
    else:
        lines.append(f"*Rate:* auto (live) — ≈₹{get_usd_inr_rate():.2f} / $1")
    lines.append("")
    lines.append("*Proceeds*")
    lines.append(f"• Today: ₹{stats['today']:.2f} ({stats['today_count']} orders)")
    lines.append(f"• This week: ₹{stats['week']:.2f} ({stats['week_count']} orders)")
    lines.append(f"• All-time: ₹{stats['total']:.2f} ({stats['total_count']} orders)")
    return "\n".join(lines)

def _crypto_mgmt_keyboard():
    enabled = crypto_payments_enabled()
    mode = cfg("crypto_rate_mode", "auto")
    rows = [
        [InlineKeyboardButton("🔴 Disable" if enabled else "🟢 Enable", callback_data="cryptomgmt:toggle")],
        [
            InlineKeyboardButton(("✅ " if mode == "auto" else "") + "🔄 Auto Rate", callback_data="cryptomgmt:rateauto"),
            InlineKeyboardButton(("✅ " if mode == "custom" else "") + "✏️ Custom Rate", callback_data="cryptomgmt:ratecustom"),
        ],
        [InlineKeyboardButton("⬅️ Back", callback_data="admin:settings")],
    ]
    return InlineKeyboardMarkup(rows)

def cfg_crypto_mgmt_cb(update: Update, context: CallbackContext):
    if update.effective_user.id != OWNER_ID:
        return
    q = update.callback_query
    q.answer()
    _clear_owner_settings_capture_state(context)
    q.message.reply_text(_crypto_mgmt_text(), parse_mode=ParseMode.MARKDOWN, reply_markup=_crypto_mgmt_keyboard())

def crypto_mgmt_cb(update: Update, context: CallbackContext):
    if update.effective_user.id != OWNER_ID:
        return
    q = update.callback_query
    data = q.data or ""

    if data == "cryptomgmt:toggle":
        set_cfg("crypto_enabled", not crypto_payments_enabled())
        q.answer("Crypto payments " + ("enabled." if crypto_payments_enabled() else "disabled."))
        _edit_ignore_unmodified(q.edit_message_text, _crypto_mgmt_text(), parse_mode=ParseMode.MARKDOWN, reply_markup=_crypto_mgmt_keyboard())
        return

    if data == "cryptomgmt:rateauto":
        set_cfg("crypto_rate_mode", "auto")
        q.answer("Switched to live rate.")
        _edit_ignore_unmodified(q.edit_message_text, _crypto_mgmt_text(), parse_mode=ParseMode.MARKDOWN, reply_markup=_crypto_mgmt_keyboard())
        return

    if data == "cryptomgmt:ratecustom":
        q.answer()
        _clear_owner_settings_capture_state(context)
        context.user_data["__await_crypto_rate__"] = True
        current = cfg("crypto_custom_rate", DEFAULT_USD_INR_RATE)
        q.message.reply_text(
            "Send the fixed rate as ₹ per $1 (e.g. `96`).\n"
            f"Current custom rate: ₹{float(current):.2f}",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    ...
    def _edit_builder():
        try:
            ...
            if _is_not_modified_error(e):
                return  # re-clicked the already-selected option; nothing to change
            q.message.reply_text(
                _special_post_builder_text(context),
                parse_mode=ParseMode.HTML,
                reply_markup=_special_post_builder_keyboard(context),
...
def admin_text_router(update: Update, context: CallbackContext):
    ...
    # (3b) Crypto custom rate setter - OWNER ONLY
    if context.user_data.pop("__await_crypto_rate__", False):
        if user_id != OWNER_ID:
            update.message.reply_text("❌ Only the bot owner can modify the crypto rate.")
            return
        try:
            rate = float(t)
            assert rate > 0
        except Exception:
            update.message.reply_text("Invalid rate. Send a positive number, e.g. `96`.", parse_mode=ParseMode.MARKDOWN)
            context.user_data["__await_crypto_rate__"] = True
            return
        set_cfg("crypto_custom_rate", rate)
        set_cfg("crypto_rate_mode", "custom")
        update.message.reply_text(f"✅ Custom crypto rate set to ₹{rate:.2f} / $1.")
        _settings_refresh(update.effective_chat.id, context)
        return
...
def editln_cb(update: Update, context: CallbackContext):
    ...
    def _show_menu():
        ...
        try:
            ...
            if _is_not_modified_error(e):
                return  # re-clicked an option that changes nothing; menu already current
...
def _on_dispatcher_error(update: Update, context: CallbackContext):
    """Global error handler: 'Message is not modified' double-clicks are
    harmless noise; everything else gets logged with a traceback instead of
    the dispatcher's 'No error handlers are registered' dump."""
    err = context.error
    if _is_not_modified_error(err):
        log.debug(f"Ignored not-modified edit: {err}")
        return
...
def main():
    ...
    # The pay window grew from 5 to 10 minutes; a previously-stored shorter QR
    # timeout would delete the QR mid-session. Never let it undercut the window.
    try:
        if int(cfg("qr_unpaid_delete_minutes", PAY_WINDOW_MINUTES)) < PAY_WINDOW_MINUTES:
    ...
    if cfg("crypto_enabled") is None: set_cfg("crypto_enabled", False)
    if cfg("crypto_rate_mode") is None: set_cfg("crypto_rate_mode", "auto")

    # Back-compat: sessions created by the previous bot version lack the new
    # claim-guard fields; stamp them so an in-flight payment still fulfills
    # across the restart. No-op when there's nothing to migrate.
    try:
        c_sessions.update_many(
            {"status": {"$exists": False}},
            {"$set": {"status": "pending", "active_view": "upi", "crypto": None}}
        )
    except Exception as e:
        log.warning(f"Legacy session stamp failed: {e}")
    ...
    dp.add_error_handler(_on_dispatcher_error)
    ...
    # Crypto payments (NOWPayments, USDT-BEP20 only) -- toggle button + admin management
    dp.add_handler(CallbackQueryHandler(pay_toggle_cb, pattern=r"^paytoggle:(upi|crypto)$"), group=1)
    dp.add_handler(CallbackQueryHandler(cfg_crypto_mgmt_cb, pattern=r"^cfg:cryptomgmt$"), group=1)
    dp.add_handler(CallbackQueryHandler(crypto_mgmt_cb, pattern=r"^cryptomgmt:(toggle|rateauto|ratecustom)$"), group=1)
    ...
    updater.job_queue.run_once(_resume_pending_crypto_polls, when=2)
